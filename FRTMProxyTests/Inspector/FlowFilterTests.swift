...
struct FlowFilterTests {
    ...
    @Test("La cache conserva gli stessi risultati del filtro stateless")
    func cachedProjectionEquivalence() {
        let cache = FlowFilter.Cache()
        var filter = FlowFilter()

        for search in ["host:api.example.com", "protocol:graphql", "secret", "type:json"] {
            filter.searchText = search
            let expected = filter.apply(to: flows).map(\.id)
            let cached = filter.apply(to: flows, using: cache).map(\.id)
            #expect(cached == expected)
        }

        var changedFlows = flows
        changedFlows[3].request = .init(
            method: "POST",
            url: "https://api.example.com/graphql",
            headers: ["content-type": "application/json"],
            body: #"{"data":{"profile":null}}"#
        )
        filter.searchText = "protocol:graphql"
        #expect(filter.apply(to: changedFlows, using: cache).isEmpty)
    }

    @Test("Il worker seriale conserva risultati e IP disponibili")
    func serialWorkerProjection() async throws {
        let worker = InspectorFlowFilterWorker()
        var filter = FlowFilter()
        filter.searchText = "host:api.example.com"

        let projection = try await worker.project(flows: flows, filter: filter)

        #expect(projection.flows.map(\.id) == ["1", "2", "4"])
        #expect(projection.clientIPs == ["127.0.0.1", "192.168.1.5"])
    }
}
